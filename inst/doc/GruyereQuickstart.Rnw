...
This model is synthesis between allometric 
...
\subsection{Visitors}
Zero or more visitor objects are shown time series chunks of the simulation as 
it is run. A visitor could potentially do anything with a chunk of the time 
series. There are a few visitors that give textual feedback and some that gives 
graphical feedback. This example uses TimeSimulationVisitor(), which prints 
the time elapsed once the simulation completes. There are also visitors for 
recording the time series chunks. CollectChunksVisitor(), which we use here, 
records all the chunks in memory. Another visitor, WriteChunksVisitor(), 
...
\begin{enumerate}
  ...
  \item show the chunk to each of  the visitors
...
\subsection{Visitors and results}
We can use the time series assembled by the CollectChunksVisitor() object. 
